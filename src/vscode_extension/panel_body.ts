// Pure body markup for the diagram panel webview.
//
// No `vscode` import — this module is the single source of truth for the
// diagram DOM and is loaded by both the production htmlShell wrapper and
// the Playwright harness generator. Anything that needs vscode-coupled
// values (script URI, CSP) lives in panel_html.ts, which wraps this body.

export function panelBody(): string {
  return `<style>
      html, body {
        margin: 0; height: 100%;
      ...
      #session-button {
        position: fixed; top: 12px; right: 200px; z-index: 10;
        background: var(--vscode-editorWidget-background);
        color: var(--vscode-descriptionForeground);
        border: 1px solid var(--vscode-widget-border, var(--vscode-panel-border));
        border-radius: 6px;
      ...
      #session-button:hover { background: var(--vscode-toolbar-hoverBackground); }
      #session-button.hidden { display: none; }
      #session-button.connected { color: var(--vscode-testing-iconPassed, #4ec9b0); }
      #settings-button {
      ...
      #help-button {
        ...
        border: 1px solid var(--vscode-statusBarItem-warningBackground, #d18616);
        border-radius: 6px;
        padding: 6px 12px; cursor: pointer;
        font-family: inherit; font-size: 13px; font-weight: 600; line-height: 1;
      }
      #help-button:hover { filter: brightness(1.1); }
      #model-picker {
        position: fixed; top: 12px; right: 330px; z-index: 10;
        background: var(--vscode-editorWidget-background);
        color: var(--vscode-foreground);
        border: 1px solid var(--vscode-widget-border, var(--vscode-panel-border));
        border-radius: 6px;
      ...
      #model-picker:hover { background: var(--vscode-toolbar-hoverBackground); }
      ...
      #modal-buttons #modal-ok:hover { background: var(--vscode-button-hoverBackground); }
      #mode-toggle {
      ...
      #zoom-slider { width: 160px; cursor: pointer; accent-color: var(--vscode-button-background); }
      #zoom-label {
      ...
      svg [data-component-id], svg [data-relationship-group] { transition: opacity 0.15s ease; }
      svg [data-relationship-group].pv-faded { opacity: 0.12; }
      svg [data-component-id].pv-faded { opacity: 0.28; }
      ...
      #legend-header {
        display: flex; align-items: center; gap: 8px;
        padding: 6px 8px 6px 10px; cursor: grab;
      ...
      #legend-toggle {
        ...
        font-family: inherit; font-size: 14px; line-height: 1; min-width: 20px;
      }
      #legend-toggle:hover { background: var(--vscode-toolbar-hoverBackground); }
      #legend-body { padding: 6px 10px 8px 10px; overflow-y: auto; min-width: 0; }
      ...
      .legend-row {
        display: flex; align-items: center; gap: 8px;
        padding: 2px 0; line-height: 1.4; min-width: 0;
      }
      .legend-row svg { flex-shrink: 0; }
      .legend-row span { min-width: 0; overflow-wrap: anywhere; }
    </style>
    <div id="stage"></div>
    <div id="empty">Loading…</div>
    <select id="model-picker" class="hidden" title="Switch which .oxford/*.json the panel renders"></select>
    <button id="session-button" class="hidden" type="button" title="Connect to a Claude Code session">⊘ No session</button>
    <button id="settings-button" class="hidden" type="button" title="Open little-oxford settings">⚙</button>
    <button id="help-button" type="button" title="Need help? Email Marcus">? Help</button>
